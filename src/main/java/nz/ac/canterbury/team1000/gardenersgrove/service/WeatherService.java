 ...
 * automatically with @link{WeatherRepository}. Also is responsible for calling the Open-Meteo API
...
public class WeatherService {
	...
	private final String FUTURE_URL =
		"https://api.open-meteo.com/v1/forecast?daily=sunrise,sunset,weather_code,temperature_2m_max,"
			+ "temperature_2m_min,precipitation_probability_max&start_date=" + LocalDate.now()
			+ "&end_date=" + dateThreeDaysLater;
...
//    }
	...
	/**
	 * Retrieves the current weather and the weather immediately before or after the current time
	 * for a given garden. This method checks the list of weather data for a garden and identifies
	 * the weather condition that is currently in effect.
	 *
	 * @param gardenId the ID of the garden for which to retrieve the weather information
	 * @return a list of Weather objects containing the current weather - the weather for previous
	...
	public List<Weather> getCurrentWeatherByGardenId(Long gardenId) {
		...
		for (int i = 0; i < weathers.size(); i++) {
			...
			if (weatherDateTime.isBefore(timeRightNow.plusMinutes(1)) && nextWeatherHour.isAfter(
				timeRightNow)) {
				if (i == 0) {
	...
	/**
	 * Retrieves the weather data for the future 3 days for a specified garden by its ID.
	 *
	 * @param gardenId the ID of the garden for which to retrieve the weather data
	 * @return a list of Weather objects containing the weather data for the garden for the future 3
	 * days
	 */
	...
	/**
	 * Retrieves the weather data for a specified garden by its ID, including weather codes,
	 * temperatures, humidity, and timestamps. The weather data is fetched from an API and parsed
	...
	public List<Weather> getWeather(Long gardenId, String url) {
		...
		try {
			...
			String newUrl = url + "&latitude=" + latitude + "&longitude=" + longitude
			...
			List<Integer> hourlyHumidity = (ArrayList) ((Map<String, Object>) weather.get(
				"hourly")).get("relative_humidity_2m");
			List<String> hourlyTime = (ArrayList) ((Map<String, Object>) weather.get("hourly")).get(
				"time");
			...
			for (int i = 0; i < weatherCodes.size(); i++) {
				weatherList.add(new Weather(gardenId, hourlyTimeParsed.get(i),
					WeatherType.getByCode(weatherCodes.get(i)),
					dailyTemps.get(i), dailyHumidity.get(i),
					hourlyTimeParsed.get(i).getDayOfWeek().toString()));
			}
	...
	/**
	 * A method that retrieves the weather data for the future 3 days for a specified garden by its
	 * ID. The weather data is fetched from an API and parsed into a list of Weather objects.
	 *
	 * @param gardenId the ID of the garden for which to retrieve the weather data
	...
	public List<Weather> getWeatherFuture(Long gardenId, String url) {
		...
		try {
			...
			List<Integer> weatherCodesFuture = (ArrayList) ((Map<String, Object>) weather.get(
				"daily")).get("weather_code");
			List<Double> maxTempFuture = (ArrayList) ((Map<String, Object>) weather.get(
				"daily")).get("temperature_2m_max");
			List<Double> minTempFuture = (ArrayList) ((Map<String, Object>) weather.get(
				"daily")).get("temperature_2m_min");
			List<Integer> precipitationFuture = (ArrayList) ((Map<String, Object>) weather.get(
				"daily")).get("precipitation_probability_max");
			List<String> futureDate = (ArrayList) ((Map<String, Object>) weather.get("daily")).get(
				"time");
			List<String> sunRise = (ArrayList) ((Map<String, Object>) weather.get("daily")).get(
				"sunrise");
			List<String> sunSet = (ArrayList) ((Map<String, Object>) weather.get("daily")).get(
				"sunset");
