...
public class FriendsController {
	 ...
	 * @param search            the user input that they are searching for, which is optional and
	...
	public String searchFriend(
		@ModelAttribute("searchFriendsForm") SearchFriendsForm searchFriendsForm,
		...
		if (!searchingByEmail && !searchingByName) {
			bindingResult.addError(
				new FieldError("searchFriendsForm", "search", searchFriendsForm.getSearch(), false,
					null, null, "Please enter a valid name or email"));
			return templateString;
		}
		...
		if (userResults.isEmpty()) {
			if (searchingByName) {
				bindingResult.addError(
					new FieldError("searchFriendsForm", "search", searchFriendsForm.getSearch(),
						false, null, null, "There is nobody with that name in Gardener’s Grove"));
			} else if (searchingByEmail) {
				bindingResult.addError(
					new FieldError("searchFriendsForm", "search", searchFriendsForm.getSearch(),
						false, null, null, "There is nobody with that email in Gardener’s Grove"));
			}
			return templateString;
		}
		...
		if (iAmOnlyResult) {
			if (searchingByName) {
				bindingResult.addError(
					new FieldError("searchFriendsForm", "search", searchFriendsForm.getSearch(),
						false, null, null,
						"There is nobody else with that name in Gardener’s Grove"));
			} else if (searchingByEmail) {
				bindingResult.addError(
					new FieldError("searchFriendsForm", "search", searchFriendsForm.getSearch(),
						false, null, null,
						"You've searched for your own email. Now, let's find some friends!"));
			}
			return templateString;
		}
		...
		for (User user : userResults) {
			FriendRelationship receivedRelationship = friendRelationshipService.getFriendRelationship(
				user.getId(), currentUser.getId());
			FriendRelationship sentRelationship = friendRelationshipService.getFriendRelationship(
	...
	public String getFriendsPage(Model model) {
		...
		List<FriendRelationship> incomingRequests = friendRelationshipService.getRelationshipsByReceiverIdAndStatus(
			loggedInUser.getId(), PENDING);
		...
		List<FriendRelationship> receivedFriends = friendRelationshipService.getRelationshipsByReceiverIdAndStatus(
			loggedInUser.getId(), Status.APPROVED);
		List<FriendRelationship> sentFriends = friendRelationshipService.getRelationshipsBySenderIdAndStatus(
			loggedInUser.getId(), Status.APPROVED);
		...
		List<FriendRelationship> outgoingPendingRequests = friendRelationshipService.getRelationshipsBySenderIdAndStatus(
			loggedInUser.getId(), PENDING);
		List<FriendRelationship> outgoingDeclinedRequests = friendRelationshipService.getRelationshipsBySenderIdAndStatus(
			loggedInUser.getId(), DECLINED);
	...
	public String postFriendRequest(@RequestParam("receiver") String receiver,
		...
		FriendRelationship existing = friendRelationshipService.getFriendRelationship(
			currentUser.getId(), receiverUser.getId());
		Boolean notExisting = existing == null;

		if (notExisting) {
			logger.info("Adding new");
			FriendRelationship newRequest = new FriendRelationship(currentUser, receiverUser,
				PENDING);
	...
	public String postDeleteFriend(@RequestParam("receiver") String receiver) {
		logger.info("POST /deleteFriend " + receiver);

		User currentUser = userService.getLoggedInUser();
		User receiverUser = userService.findEmail(receiver);

		logger.info("Deleting friend request " + receiverUser.getEmail() + " and "
			+ currentUser.getEmail());
//		logger.info("Deleting friend request " + receiverUser.getEmail());
//		logger.info(receiverUser.getEmail(), currentUser.getEmail());
		friendRelationshipService.cancelFriendRelationship(currentUser.getId(),
			receiverUser.getId());
	...
	public String postDeclineFriend(@RequestParam("senderUserId") Long senderUserId,
		...
		friendRelationshipService.updateFriendRelationship(senderUserId, loggedInUser.getId(),
			Status.DECLINED);

		List<FriendRelationship> requests = friendRelationshipService.getRelationshipsByReceiverIdAndStatus(
			loggedInUser.getId(), PENDING);
		List<User> requestingUsers = new ArrayList<>();
